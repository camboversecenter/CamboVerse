/**
 * public/models/angkor-wat.glb — an authored, recognizable Angkor Wat: the
 * five-tower quincunx (tall central prang + four corner towers with lotus-bud
 * finials), tiered galleries and a causeway. Uses the shared temple builder
 * (detailed towers + weathered vertex colours). Stylized stand-in; the photoreal
 * version will be a Gaussian-splat capture. Run: `node scripts/generate-angkor.mjs`
 */
...
// Gallery walls with a repeated cornice line.
for (const [x, z, w, d] of [[0, 2.35, 5.6, 0.22], [0, -2.35, 5.6, 0.22], [2.7, 0, 0.22, 4.9], [-2.7, 0, 0.22, 4.9]]) {
  b.box(w, 0.7, d, x, 1.35, z, SAND);
  b.box(w * 1.03, 0.12, d * 1.03, x, 1.72, z, DARK_SAND); // cornice
}

// Causeway (entrance, front = +z) with balustrades.
...
const py = 1.4;
b.tower(0, 0, 1.5, py, 4.0, 8); // central, tallest
