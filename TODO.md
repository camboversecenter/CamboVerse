...
- **🏙️ District (ADM2) boundaries** *(shipped)*. The province map is now
  subdivided into its 197 districts (`khm_admbnda_adm2`), generated by
  `scripts/generate-districts.mjs` → `src/cambodia-districts.ts` (same map plane,
  lazy-loaded). Tap a district to **filter to what's inside it** — its heritage
  sites and living farms. Room to grow: **district labels/search**, per-district
  Khmer names, and **commune (ADM3)** boundaries as a fourth tier.
