/*
 * VENDORED — Grove reference verifier.
 * Source of truth: sengtha/iAny → grove/core/grove.ts (protocol: grove/SPEC.md).
 * Copied verbatim into CamboVerse so the viewer is self-contained (no external
 * runtime dependency) and can verify every record itself. Licence: Apache-2.0.
 * Do not diverge from SPEC.md §§4–5 — canonical JSON → SHA-256 → ECDSA-P256.
 * Re-sync from upstream if the spec revises. Web Crypto only; runs in browser,
...
/** UTF-8 bytes as an ArrayBuffer-backed view. Behaviour is identical to
 *  `enc.encode`; the explicit `Uint8Array<ArrayBuffer>` type satisfies Web
 *  Crypto's `BufferSource` under TS 5.7's stricter Uint8Array generics. */
function utf8(s: string): Uint8Array<ArrayBuffer> {
  return new Uint8Array(enc.encode(s))
}

export async function sha256Hex(input: string | ArrayBuffer): Promise<string> {
  const buf = typeof input === 'string' ? utf8(input) : input
...
export async function signAttestation(
  ...
  const id = await sha256Hex(canonicalize(unsigned))
  const sigBuf = await crypto.subtle.sign({ name: 'ECDSA', hash: 'SHA-256' }, key.privateKey, utf8(id))
  return { ...unsigned, id, sig: b64url(new Uint8Array(sigBuf)) }
}
...
export async function verifyAttestation(a: Attestation): Promise<VerifyResult> {
  ...
  try {
    ...
    sigOk = await crypto.subtle.verify({ name: 'ECDSA', hash: 'SHA-256' }, pub, unb64url(sig), utf8(id))
  } catch {
    /* malformed */
  }
  return { ok: idOk && sigOk, idOk, sigOk }
