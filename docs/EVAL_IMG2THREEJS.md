...
Both halves of the verdict have been acted on:

- **The observation protocol is now step 2 of
  [`docs/BUILDINGS.md`](./BUILDINGS.md)** — observe before you infer, decompose
  macro→meso→micro, name three identity-defining features, write down what one
  photo cannot show, and overlay a grid before you commit to numbers. No install,
  no dependency.
- **The two errors it caught are fixed in `GreatHall`
  (`src/components/CampusBuildings.tsx`).** The glazing now stops short of the
  soffit with an open shaded storey above it, read through the colonnade, and the
  eave cantilevers 6 m rather than 4.5. The colonnade posts and rooftop plant are
  instanced while we were in there, taking the hall from 72 meshes to 26.

Nothing generated by the tool was merged. The model in the repo is still the
hand-written one — corrected by what the process surfaced.
